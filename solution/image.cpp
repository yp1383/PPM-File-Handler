...
void substractPPM(const std::string &filename, const PPM::PPMObject &image1, const PPM::PPMObject &image2){
    try{
        std::ofstream savePPMFile(filename, std::ofstream::trunc);
        savePPMFile << image1.PType << "\n"
            << image1.mRow << " " << image1.mCol << "\n"
...
int main(int argc, char* argv[]) {
    ...
    savePPM("first.ppm", *image1);
    gamma_corrected(0.25f, *image1);
    savePPM("second.ppm", *image1);
    gamma_corrected(0.25f, *image1);
    savePPM("third.ppm", *image1);
    return 0;
}